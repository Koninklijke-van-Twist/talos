 ...
 * Mímir beheert environments — Talos heeft alleen de API-key (+ optioneel $mimirBase) nodig.
 *
...
function odata_mimir_parse_entity_url(string $url): ?array
{
    ...
    // Talos codeert Company('…') als Company%28%27…%27%29. Eén decode dekt beide vormen.
    $path = rawurldecode((string) $parts['path']);
    if (preg_match("#/ODataV4/Company\\('(.*)'\\)/([^/?]+)#i", $path, $match) !== 1) {
        return null;
    }
    $company = str_replace("''", "'", $match[1]);
    $entity = rawurldecode($match[2]);
    $query = [];
...
function odata_mimir_parse_companies_url(string $url): ?array
{
    ...
    $path = rawurldecode((string) $parts['path']);
    // .../{environment}/ODataV4/Company of Companies — niet Company('naam')/entity.
 ...
 * $odataQuery gebruikt Talos-keys zoals $select / $filter.
 *
...
function odata_get_all(string $url, array $auth, $ttlSeconds = 300): array
{
    ...
    if (odata_mimir_api_key() !== '') {
        // Mímir beheert de BC-cache (max_age); Talos-filecache wordt overgeslagen.
